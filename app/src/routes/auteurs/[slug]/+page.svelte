<script lang="ts">
	...
	let cosigPhotoErrors = $state<boolean[]>(data.cosignataires.map(() => false));

	const photoUrl = data.dep?.photo
	...
	const groupeColour = data.groupe?.couleur ?? '#6b7280';
</script>
...
<svelte:head>
	...
	<meta
		name="description"
		content="Profil de {data.name}, député·e à l'Assemblée nationale. {data.posts
			.length} propositions de loi."
	/>
</svelte:head>
				...
				<div class="shrink-0">
					{#if photoUrl && !photoError}
						<img
							src={photoUrl}
							alt={data.name}
							class="h-24 w-24 rounded-full bg-accent object-cover ring-4 ring-border sm:h-28 sm:w-28"
							onerror={() => {
								photoError = true;
							}}
						/>
			...
			<div class="mt-6 overflow-hidden rounded-xl border border-border/70 bg-accent/20">
				...
				<div class="flex divide-x divide-border/70">
					<div class="flex-1 px-4 py-3 text-center">
						<p class="text-2xl font-extrabold tabular-nums text-foreground">{data.posts.length}</p>
						<p class="mt-0.5 text-[10px] uppercase tracking-wider text-muted-foreground">
							Proposition{data.posts.length > 1 ? 's' : ''}
						</p>
					</div>
	...
	<div class="grid grid-cols-1 gap-8 lg:grid-cols-[1fr_240px]">
		<section>
			<h2 class="mb-4 text-xs font-semibold uppercase tracking-widest text-muted-foreground">
				Propositions ({data.posts.length})
			</h2>
